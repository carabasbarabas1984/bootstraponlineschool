class NewsList {
    constructor(newsUrl, renderContainer, cart) {
        this.cart = cart;
        fetch(newsUrl)
            ...
            productListDomString +=
                `<div class="col-12 col-sm-6 col-md-4 col-lg-3 mb-3">
                  <div class="card product">
                    <img class="card-img-top" src="img/siteNews/${product.image}" 
                        alt="${product.title}">
                    <div class="card-body">
                      <h4 class="card-title">${product.title}</h4>
                      <button class="btn btn-info" data-toggle="modal"
                        data-target="#productInfoModal" data-id="${product.id}">Читати
                      </button>
                    </div>
                  </div>
                </div>`;
    ...
    addEventListeners() {
        $('#productInfoModal').on('show.bs.modal', event => {
            ...
            modal.find('.modal-body .card-img-top')
                .attr('src', 'img/siteNews/' + product.image)
